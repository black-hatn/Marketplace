  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/5] overflow-hidden rounded-[2rem] bg-slate-100 dark:bg-slate-900">
        ...
        />
        <div className="absolute inset-0 bg-black/20 dark:bg-black/40 opacity-0 transition-opacity duration-300 group-hover:opacity-100 flex flex-col items-center justify-center gap-4 p-4 z-20">
        ...
        {product.badge && (
          <span className="absolute left-4 top-4 rounded-full bg-cyan-500/10 dark:bg-cyan-500/15 px-3 py-2 text-xs font-bold uppercase tracking-[0.25em] text-cyan-700 dark:text-cyan-100 shadow-lg backdrop-blur-md z-20">
            {product.badge}
          </span>
        )}
      </div>
      ...
      <div className="flex flex-1 flex-col gap-6 p-6">
        ...
        <div>
          <h3 className="mt-4 text-2xl font-semibold text-slate-900 dark:text-white line-clamp-1">{title}</h3>
          {product.tagline && <p className="mt-3 text-sm leading-7 text-slate-600 dark:text-slate-300 line-clamp-2">{product.tagline}</p>}
        </div>
        <div className="mt-auto space-y-4">
          <div className="flex items-center justify-between gap-4 text-slate-700 dark:text-slate-200">
            ...
            <span className="text-xl font-semibold text-slate-900 dark:text-white">{price.toLocaleString()} FCFA</span>
          </div>
          <div className="inline-flex w-full items-center justify-center rounded-full bg-cyan-400 px-5 py-3 text-sm font-semibold text-slate-950 transition hover:bg-cyan-300 shadow-md">
            Détails
          </div>
        </div>
      </div>
