...
export function Footer() {
  ...
  return (
    <footer className="relative bg-white dark:bg-[#030303] border-t border-black/5 dark:border-white/5 overflow-hidden">
      {/* Glow Effects */}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {trustBadges.map((badge) => (
              <div key={badge.label} className="flex items-center gap-4 group">
                <div className="w-16 h-16 rounded-3xl bg-slate-100 dark:bg-white/5 flex items-center justify-center text-slate-400 dark:text-white/40 group-hover:text-cyan-600 dark:group-hover:text-cyan-400 group-hover:scale-110 group-hover:rotate-12 transition-all duration-700 shadow-lg border border-black/5 dark:border-white/5">
                  <badge.icon className="w-7 h-7" />
                </div>
                <div className="space-y-1">
                  <p className="text-[11px] font-black text-slate-950 dark:text-white uppercase tracking-[0.2em]">{badge.label}</p>
                  <p className="text-[9px] font-black text-slate-400 dark:text-white/20 uppercase tracking-widest leading-none">{badge.sub}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="max-w-[1400px] mx-auto px-6 sm:px-12 pt-20 pb-12 relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-16 lg:gap-12">
          ...
          <div className="lg:col-span-2 space-y-8">
            <Link href="/" className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-xl font-black text-slate-950 dark:text-white tracking-tighter uppercase italic">Immersive<span className="text-cyan-600">.</span></h2>
                <p className="text-[9px] font-black text-slate-400 dark:text-white/20 uppercase tracking-[0.4em]">Marketplace Pro</p>
              </div>
            </Link>
            ...
            <div className="flex items-center gap-3">
              {[Instagram, Twitter, Facebook, Youtube].map((Icon, i) => (
                <a key={i} href="#" className="w-10 h-10 rounded-full glass flex items-center justify-center text-slate-500 dark:text-white/40 hover:text-cyan-500 dark:hover:text-white hover:bg-white/10 transition-all border border-black/5 dark:border-white/10">
          ...
          {/* Links Columns */}
          <div>
            <h3 className="text-[10px] font-black text-slate-950 dark:text-white uppercase tracking-[0.4em] mb-8">Navigation</h3>
            <ul className="space-y-4">
              {footerLinks.boutique.map((link) => (
                <li key={link.label}>
                  <Link href={link.href} className="text-sm text-slate-600 dark:text-white/60 hover:text-cyan-600 dark:hover:text-white transition-colors font-medium">
                    {link.label}
                  </Link>
                </li>
              ))}
          ...
          <div>
            <h3 className="text-[10px] font-black text-slate-950 dark:text-white uppercase tracking-[0.4em] mb-8">Assistance</h3>
            <ul className="space-y-4">
          ...
          <div>
            <h3 className="text-[10px] font-black text-slate-950 dark:text-white uppercase tracking-[0.4em] mb-8">Informations</h3>
            <ul className="space-y-4">
        ...
        {/* Newsletter Bar */}
        <div className="mt-20 p-10 bg-slate-950 dark:bg-white/[0.03] rounded-[3rem] border border-black/5 dark:border-white/10 flex flex-col md:flex-row items-center justify-between gap-8">
          <div>
            <h3 className="text-xl font-black text-white uppercase tracking-tighter italic">Rejoindre l&apos;Insider<span className="text-cyan-500">.</span></h3>
            <p className="text-sm text-slate-400 font-medium mt-1">Recevez nos dernières collections en avant-première.</p>
          </div>
          <form className="flex w-full md:w-auto gap-3" onSubmit={(e) => e.preventDefault()}>
            <input 
              type="email" 
              placeholder="votre@email.com"
              className="flex-1 md:w-64 px-6 py-4 rounded-2xl bg-white/10 border border-white/10 text-white placeholder-white/30 outline-none focus:border-cyan-500 text-sm font-bold" 
            />
            <button className="px-8 py-4 rounded-2xl bg-white text-black font-black uppercase tracking-widest text-[10px] hover:scale-105 transition-all">
              S&apos;abonner
            </button>
          </form>
        </div>
