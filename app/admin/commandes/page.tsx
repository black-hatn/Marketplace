...
export default async function OrdersPage() {
  ...
  const countByStatus = (status: string) => orders.filter((o: any) => o.status === status).length;
        ...
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 justify-between items-center">
            ...
            <div className="flex items-center gap-3">
              ...
              <Link href="/" className="flex items-center gap-1 text-sm font-medium text-slate-500 hover:text-slate-900 dark:hover:text-white transition-colors">
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            { label: 'En attente', value: countByStatus('PENDING'), class: 'text-amber-600 dark:text-amber-400' },
            { label: 'Payées', value: countByStatus('PAID'), class: 'text-blue-600 dark:text-blue-400' },
            { label: 'Expédiées', value: countByStatus('SHIPPED'), class: 'text-violet-600 dark:text-violet-400' },
          ].map((s) => (
                    ...
                    return (
                      <tr key={order.id} className="hover:bg-slate-50/80 dark:hover:bg-slate-900/40 transition-colors">
                        ...
                        <td className="px-5 py-4 hidden sm:table-cell">
                          <span className="text-sm text-slate-600 dark:text-slate-400">{order.items.length} article{order.items.length > 1 ? 's' : ''}</span>
                        </td>
                        <td className="px-5 py-4">
                          <span className="text-sm font-bold text-slate-900 dark:text-white">{order.total.toLocaleString()} FCFA</span>
                        </td>
                        <td className="px-5 py-4">
