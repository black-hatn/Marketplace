                    ...
                    <div className="p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-mono bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-lg text-slate-500 uppercase tracking-wider">#{order.id.slice(-8)}</span>
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full uppercase tracking-tighter ${order.status === 'PAID' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400'}`}>
                      ...
                      <div className="flex items-center gap-6">
                        <div className="text-right">
                          ...
                          <p className="text-xl font-bold text-cyan-600">{order.total.toLocaleString()} FCFA</p>
                        </div>
                      ...
                      <div className="flex gap-4 overflow-x-auto pb-2">
                        {order.items.map((item: any) => (
                          <div key={item.id} className="flex-shrink-0 group">
                            ...
                            <p className="text-[10px] text-center mt-1 text-slate-500">×{item.quantity}</p>
                          </div>
                        ))}
                      </div>
                    </div>
